...
function get_mans(){
	...
	function save_man(){
		...
		$file_namedl=$func->images_name($_FILES['filedl']['name']);
		if($photo_dl = $func->upload_image("filedl",_download_type, _upload_download,$file_namedl)){
			$data['download'] = $photo_dl;
			if($id){
				$db->setTable('post');
				$db->setType('row');
				$db->setWhere('id', $id);
				$row = $db->select('download');
				if($row){
					$func->delete_file(_upload_download.$row['download']);
				}
			}	
		}
		...
		$data['datein'] = $_POST['datein'];
		$data['code'] = $_POST['code'];
		$data['time'] = $_POST['time'];
		$data['phuongtien'] = $_POST['phuongtien'];
		$data['diemkhoihanh'] = $_POST['diemkhoihanh'];
		$data['diemden'] = $_POST['diemden'];
	...
	function delete_man(){
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			$db->bindMore(array("id"=>$id));
			$row = $db->row("select id,photo,thumb,download from #_post where id=:id ");
