...
if($_GET['w'] < 400){
	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(__FILE__) . '/upload/watermark_thumb.png' ); // the watermark image
} else {
	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(__FILE__) . '/upload/watermark.png' ); // the watermark image
}
...
class timthumb {
	...
	public function __construct(){
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ""))){
