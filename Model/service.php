<?php
	...
	@$id =  $_GET['id'];
	#các sản phẩm khác======================
	$select_field = "name_$lang,slug,id,thumb,photo,description_$lang,datecreate,view,type,time,code,phuongtien,oldprice,price,datein";
	if($id!=''){

		$responsiveslides = new \Library\plugins('Slider','responsiveslides');
		$plugin_css .= $responsiveslides->css();
		$plugin_js .= $responsiveslides->js();
		...
		$list_news = $db->query("select id,slug,name_$lang from #_cate_list where shows=:shows and type=:type order by number,id desc");
